...
|---|------|--------|------------|-------|
| 29-01 | [Permission hooks: local mode](./29-01-permission-hook-local-mode_opus.md) | done | — | **opus.** Committed `58d5d44`. Guarded imports, fixed auto-allow ordering bug, added 10 tests with state-store assertions. |
| 29-02 | [Registry, manifest, probes, harness](./29-02-registry-manifest-probes.md) | done | — | Foundation for everything below. `install-claude-config.sh` frozen (§0); `install.sh` created with full 10-feature registry, MODULE_OWNERS, 3 startup assertions, manifest read/write, probes, `--only`/`--probe` flags, and real-`$HOME` guard. Test harness `tests/test_unit_installer.py` registered as `unit_installer`, 23 tests all passing. |
| 29-03 | [External-surface seam](./29-03-external-surface-seam.md) | done | 29-02 | 11 `ext_*` seam functions + `ext_systemd_daemon_reload` + `ext_cron_has_marker` added to `install.sh`. All existing callers (3 `ln -sf`, `systemctl`/`loginctl`, `tmux set -g`, crontab probes) migrated. `_install_tmux_options` delegates to `ext_tmux_apply`; `_install_questions_listen_unit` uses `ext_systemd_daemon_reload` + `ext_systemd_enable`. 35 new tests (58 total in `unit_installer`), all passing. Source-level grep invariant test passes. |
| 29-04 | [Settings ownership](./29-04-settings-ownership.md) | done | 29-02 | Committed `fcfed09`. Ownership-scoped hook merge (matcher-keyed Notification), union/overwrite permissions, MCP helper with ~/.claude.json backup. 28 new tests. |
